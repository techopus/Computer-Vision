...
def kmeans(features, k, num_iters=100):
    ...
    for n in range(num_iters):
        ...
        tmp = centers.copy()  #points assigned as center
        for j in range(k):
            centers[j] = np.mean(features[assignments == j], axis=0)  #calculate center for clusters
        if np.allclose(tmp, centers): #check if cluster assignment is same
            break
...
def kmeans_fast(features, k, num_iters=100):
    ...
    for n in range(num_iters):
        ### YOUR CODE HERE
         ...
         ##similar task done as above
        f_tmp = np.tile(features, (k, 1))   #as hinted, used both np libraries
